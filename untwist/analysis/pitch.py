...
class YINFFT(Processor):
    ...
    2500.,3150., 4000., 5000., 6300., 8000., 9000., 10000.,12500., 15000.,\
     20000., 25100]

    weight_mask = [-75.8, -70.1, -60.8, -52.1, -44.2, -37.5, -31.3, -25.6,\
     -20.9, -16.5, -12.6, -9.6, -7.0, -4.7, -3.0, -1.8, -0.8, -0.2, -0.0,\
     0.5, 1.6, 3.2, 5.4, 7.8, 8.1, 5.3, -2.4, -11.1, -12.8, -12.2, -7.4,\
     -17.8, -17.8, -17.8]
    ...
    def compute_weights(self):
        ...
        for i in range(self.n_bins):
            freq = i * bin_hz
            while(self.freq_mask[j] < freq): j+=1
            a0 = self.weight_mask[j-1]; f0 = self.freq_mask[j-1]
            a1 = self.weight_mask[j];f1 = self.freq_mask[j]
            if f0 == 0:
    ...
    def __init__(self, n_bins, sample_rate, min_pitch = 100, max_pitch = 5000, interp = True):
        self.n_bins = n_bins
        self.sample_rate = sample_rate
        self.compute_weights()
        self.tau_max = int(min(np.ceil(float(sample_rate)/ min_pitch),n_bins))
        self.tau_min = int(min(np.floor(float(sample_rate)/ max_pitch),n_bins))
        self.interp = interp
